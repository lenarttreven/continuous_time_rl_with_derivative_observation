...
if __name__ == '__main__':
    ...
    initial_conditions = [jnp.array([-jnp.pi / 6, 0.0])]
    time_horizon = (0, 1)
    noise_scalar = 0.01
    stds_for_simulation = jnp.array([0.01, 0.001], dtype=jnp.float64)
    ...
    run_config = RunConfig(
        ...
        data_generation=DataGenerationConfig(
            scaling=Scaling(state_scaling=jnp.eye(state_dim), control_scaling=jnp.eye(action_dim),
        ...
        comparator=ComparatorConfig(num_discrete_points=5)
    )
