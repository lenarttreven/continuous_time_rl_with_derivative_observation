...
def tune_params(system_params):
    ...
    return (alpha - true_alpha) ** 2 + (beta - true_beta) ** 2 + (gamma - true_gamma) ** 2 + (delta - true_delta) ** 2
...
out = minimize_ipopt(tune_params, init_params, jac=jit(grad(tune_params)), bounds=bnds, options={'maxiter': 10000})
print(out)
