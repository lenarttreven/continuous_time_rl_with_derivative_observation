...
d_l, d_u = 0, 10
xs = random.uniform(
    key=random.PRNGKey(0), shape=(10, input_dim), minval=d_l, maxval=d_u
)
ys = jnp.sin(xs)
...
test_ys_noisy = jnp.sin(test_xs) + noise_level * random.normal(
...
best_ps = model.calculate_calibration_score(
    model_params,
    model_stats,
    test_xs,
    test_ys,
...
def apply_particles(x):
    return vmap(model.apply_eval, in_axes=(0, 0, None, None))(
        model_params, model_stats, x, data_stats
    )
...
def kernel(x1, x2):
    ...
    return jnp.sum(
        (preds_1 - jnp.mean(preds_1, axis=0)[jnp.newaxis, ...])
        * (preds_2 - jnp.mean(preds_2, axis=0)[jnp.newaxis, ...]),
        axis=0,
...
preds = vmap(apply_ens, in_axes=(0, 0, None, None))(
    model_params, model_stats, potential_xs, data_stats
)
...
plt.scatter(
    potential_xs[best_indices], jnp.zeros_like(potential_xs[best_indices]), color="red"
)
plt.show()
