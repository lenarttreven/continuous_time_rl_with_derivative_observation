...
time_discretization_proposed = TimeDiscretization(
    ts=ts_proposed, ts_delta=jnp.diff(ts_proposed)
)

# First we solve problem with very high resolution to get trajectory
out = optimizer.solve(
    time_discretization_proposed,
    time_discretization_proposed,
    initial_state,
    initial_action_proposed,
    control_low=control_low,
    control_high=control_high,
    ilqr_hyperparams=ilqr_params,
    cem_hyperparams=cem_params,
    random_key=key,
)
print("Cost: ", out[2])
