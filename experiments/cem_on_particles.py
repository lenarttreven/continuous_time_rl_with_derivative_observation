...
data_stats = DataStatsBNN(input_stats=Stats(mean=jnp.mean(input_data, axis=0), std=jnp.std(input_data, axis=0)),
                          output_stats=Stats(mean=jnp.mean(output_data, axis=0), std=jnp.std(output_data, axis=0)))
...
num_particles = 10
model = DeterministicEnsemble(input_dim=input_dim, output_dim=output_dim, features=[64, 64],
...
def dynamics(x, u, t):
    ...
    return jnp.mean(vmap(model.apply_eval, in_axes=(0, 0, None, None))(model_params, model_stats, z, data_stats),
                    axis=0)
