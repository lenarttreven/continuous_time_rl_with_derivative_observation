...
def experiment(
    ...
    noise_scalar = 0.01
    my_stds_for_simulation = noise_scalar * jnp.ones(
        shape=(state_dim,), dtype=jnp.float64
    )
    ...
    def initial_control(x, t):
        return 0.1 * jnp.array(
            [jnp.sin(t).reshape(), jnp.cos(t).reshape()], dtype=jnp.float64
        )
