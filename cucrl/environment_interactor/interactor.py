...
from cucrl.environment_interactor.measurements_collector.measurements_collector import (
...
class MPCInteractor(Interactor):
    def __init__(
        ...
        self.measurements_collector = MeasurementsCollector(
            x_dim,
            u_dim,
            dynamics,
            interaction_config,
    ...
    def tree_flatten(self):
        ...
        aux_data = {
            ...
            "offline_planner": self.offline_planner,
    ...
    def tree_unflatten(cls, aux_data, children):
        (
            ...
            key,
            integration_carry,
            offline_planning_data,
        ) = children
        new_class = cls(
            x_dim=aux_data["state_dim"],
            u_dim=aux_data["control_dim"],
            dynamics=aux_data["dynamics"],
            x0s=aux_data["initial_conditions"],
            normalizer=aux_data["normalizer"],
            angle_layer=aux_data["angle_layer"],
            interaction_config=aux_data["interaction_config"],
            offline_planner=aux_data["offline_planner"],
    ...
    def update(self, dynamics_model: DynamicsModel, key: random.PRNGKey):
        ...
        mpc_carry, offline_planning_data, tracking_data = self.policy.update(
            dynamics_model, key
        )
        collector_carry = self.measurements_collector.update(dynamics_model, subkey)
        self.integration_carry = IntegrationCarry(
            mpc_carry=mpc_carry, collector_carry=collector_carry
        )
    ...
    def interact(self, x: chex.Array, t: chex.Array, traj_idx, events) -> Interaction:
        u, new_events = self.policy.apply(
            x, t, self.tracking_data, self.dynamics_model, traj_idx, events
        )
        measurement_selection, new_events = self.measurements_collector.apply(
            x, t, self.tracking_data, self.dynamics_model, traj_idx, new_events
        )
