...
class MPCTracking(Policy):
    ...
    def update_mpc_for_cond(
        ...
        new_true_policy = self.mpc_tracker.update_mpc(
            x_k, t_k, tracking_data, events.mpc_carry.mpc_params, dynamics_model
        )
        new_mpc_carry = MPCCarry(
            next_update_time=next_update_idx,
            key=subkey,
            mpc_params=events.mpc_carry.mpc_params,
            true_policy=new_true_policy,
        )
        new_events = IntegrationCarry(
            mpc_carry=new_mpc_carry, collector_carry=events.collector_carry
        )
        return new_events

    @staticmethod
    def no_update_mpc_for_cond(
        x_k: chex.Array,
        t_k: chex.Array,
        events: IntegrationCarry,
        tracking_data,
    ...
    def other_episode(
        ...
        new_events = cond(
            t_k == events.mpc_carry.next_update_time,
            self.update_mpc_for_cond,
            self.no_update_mpc_for_cond,
            x_k,
            t_k,
            events,
            tracking_data,
    ...
    def apply(
        ...
        u, new_events = cond(
            ...
            t_k,
            events,
            tree_map(lambda z: z[traj_idx], tracking_data),
            dynamics_model,
        )
        return u, new_events
    ...
    def update(self, dynamics_model: DynamicsModel, key: random.PRNGKey):
        ...
        tracking_data = TrackingData(
            ts=offline_planning_data.ts,
            xs=offline_planning_data.xs,
            us=offline_planning_data.us,
            final_t=offline_planning_data.final_t,
            target_x=offline_planning_data.target_x,
            target_u=offline_planning_data.target_u,
        )
        ...
        true_policy = TruePolicy(
            ts_idx=jnp.repeat(ts_init[jnp.newaxis, ...], repeats=self.num_traj, axis=0),
