...
from cucrl.utils.greedy_point_selection import (
    greedy_largest_subdeterminant_jit,
    greedy_distance_maximization_jit,
)
...
def get_dynamics(
    ...
    if dynamics_model == Dynamics.FSVGD_VAN_DER_POOL:
        ...
        return fSVGDDynamicsFurutaPendulum(
            state_dim,
            action_dim,
            normalizer,
            dynamics_config.features,
        ...
        return fSVGDDynamicsQuadrotorEuler(
            state_dim,
            action_dim,
            normalizer,
            dynamics_config.features,
        ...
        return BNNDynamics(
            ...
            normalizer,
            dynamics_config.features,
            bnn_type=dynamics_config.bnn_type,
            bandwidth_prior=dynamics_config.bandwidth_prior,
...
class AbstractDynamics(ABC):
    def __init__(self, x_dim: int, u_dim: int):
        self.x_dim = x_dim
        self.u_dim = u_dim
        self.mean_and_std_eval_batch = vmap(
            self.mean_and_std_eval_one,
            in_axes=(None, 0, 0),
            out_axes=(0, 0),
            axis_name="batch",
        )
    ...
    @abstractmethod
    def mean_and_std_eval_one(
        self, dynamics_model: DynamicsModel, x: jax.Array, u: jax.Array
    ...
    @abstractmethod
    def calculate_calibration_alpha(
        self,
    ...
    @abstractmethod
    def propose_measurement_times(
        self,
        dynamics_model: DynamicsModel,
        xs_potential: jax.Array,
        us_potential: jax.Array,
        ts_potential: jax.Array,
        noise_std: float,
        num_meas_array: jax.Array,
...
class BNNDynamics(AbstractDynamics):
    def __init__(
        ...
        if bnn_type == BNNTypes.DETERMINISTIC_ENSEMBLE:
            self.BNN = DeterministicEnsemble(
                input_dim=in_shape + u_dim,
                output_dim=x_dim,
                features=features,
    ...
    def calculate_calibration_alpha(
        ...
        return self.BNN.calculate_calibration_alpha(
            dynamics_model.params,
            dynamics_model.model_stats,
            zs,
            xs_dot,
            xs_dot_std,
            ps,
    ...
    def _particle_prediction(
        ...
        xs_dot = vmap(self.BNN.apply_eval, in_axes=(0, 0, None, None))(
            dynamics_model.params, dynamics_model.model_stats, z, f_data_stats
        )
        return xs_dot
    ...
    def _covariance(self, dynamics_model, x1, u1, x2, u2):
        ...
        return jnp.sum(
            (xs_dot_1 - jnp.mean(xs_dot_1, axis=0)[jnp.newaxis, ...])
            * (xs_dot_2 - jnp.mean(xs_dot_2, axis=0)[jnp.newaxis, ...]),
            axis=0,
        ) / (self.BNN.num_particles - 1)
    ...
    def propose_measurement_times(
        ...
        posterior_kernel_v = vmap(
            self._covariance, in_axes=(None, 0, 0, None, None), out_axes=1
        )
        posterior_kernel_m = vmap(
            posterior_kernel_v, in_axes=(None, None, None, 0, 0), out_axes=2
        )
        ...
        ts_potential = jnp.linspace(
            jnp.min(ts_potential),
            jnp.max(ts_potential),
        ...
        covariance_matrix = posterior_kernel_m(
            dynamics_model, xs_potential, us_potential, xs_potential, us_potential
        )

        assert covariance_matrix.shape == (
            self.x_dim,
            xs_potential.shape[0],
            xs_potential.shape[0],
        )
        covariance_matrix = (
            covariance_matrix
            + noise_std**2 * jnp.eye(xs_potential.shape[0])[None, ...]
        )

        if (
            self.measurement_collection_config.batch_strategy
            == BatchStrategy.MAX_DETERMINANT_GREEDY
        ):
            greedy_indices, potential_indices = greedy_largest_subdeterminant_jit(
                covariance_matrix, num_meas_array
            )
        elif (
            self.measurement_collection_config.batch_strategy
            == BatchStrategy.MAX_KERNEL_DISTANCE_GREEDY
        ):
            greedy_indices, potential_indices = greedy_distance_maximization_jit(
                covariance_matrix, num_meas_array
            )
        elif (
            self.measurement_collection_config.batch_strategy
            == BatchStrategy.EQUIDISTANT
        ):
            num_step = math.ceil(
                self.measurement_collection_config.num_interpolated_values
                // len(num_meas_array)
            )
            greedy_indices = jnp.arange(
                0, self.measurement_collection_config.num_interpolated_values, num_step
            ).astype(jnp.int32)

        ts_potential = ts_potential.reshape(-1, 1)
        proposed_ts = ts_potential[greedy_indices]

        initial_variances = vmap(
            self._covariance, in_axes=(None, 0, 0, 0, 0), out_axes=0
        )(dynamics_model, xs_potential, us_potential, xs_potential, us_potential)
        assert initial_variances.shape == (
            xs_potential.shape[0],
            self.x_dim,
        )
        ...
        return MeasurementSelection(
            ...
            potential_ts=ts_potential,
            potential_us=us_potential,
            potential_xs=xs_potential,
            vars_before_collection=initial_variances,
    ...
    def loss(
        ...
        input_stats = jtu.tree_map(
            ...
            data_stats.xs_after_angle_layer,
            data_stats.us_stats,
        )
        f_data_stats = DataStatsFSVGD(
            input_stats=input_stats, output_stats=data_stats.xs_dot_noise_stats
        )
        return self.BNN.loss(
            params, stats, data, f_data_stats, std_state_der, num_train_points, key
        )
...
class GPDynamics(AbstractDynamics):
    def __init__(
        ...
        self.m_kernel_multiple_output = vmap(
            self.m_kernel, in_axes=(None, None, 0), out_axes=0
        )
        self.v_kernel_multiple_output = vmap(
            self.v_kernel, in_axes=(None, None, 0), out_axes=0
        )
        self.kernel_multiple_output = vmap(
            self.kernel, in_axes=(None, None, 0), out_axes=0
        )
    ...
    def kernel(self, x, y, params):
        ...
        return jnp.exp(
            -jnp.sum((x - y) ** 2 / make_positive(params["lengthscale"]) ** 2)
        )
    ...
    def mean_and_std_eval_one(self, dynamics_model: DynamicsModel, x, u):
        ...
        expanded_x = self.normalizer.normalize(
            expanded_x, dynamics_model.data_stats.xs_after_angle_layer
        )
        ...
        expanded_xs = vmap(self.normalizer.normalize, in_axes=(0, None))(
            expanded_xs, dynamics_model.data_stats.xs_after_angle_layer
        )
        us = vmap(self.normalizer.normalize, in_axes=(0, None))(
            dynamics_model.history.us, dynamics_model.data_stats.us_stats
        )

        dot_xs = vmap(self.normalizer.normalize, in_axes=(0, None))(
            dynamics_model.history.xs_dot, dynamics_model.data_stats.xs_dot_noise_stats
        )
        std_dot_xs = vmap(self.normalizer.normalize_std, in_axes=(0, None))(
            dynamics_model.history.xs_dot_std,
        ...
        covariance_matrix = self.m_kernel_multiple_output(
            inputs, inputs, dynamics_model.params
        )
        noise_term = vmap(jnp.diag, in_axes=0)(std_dot_xs.T)
        noisy_covariance_matrix = covariance_matrix + noise_term

        k_x_X = vmap(self.v_kernel, in_axes=(None, None, 0), out_axes=0)(
            inputs, cat_input, dynamics_model.params
        )
        cholesky_tuples = vmap(jax.scipy.linalg.cho_factor)(noisy_covariance_matrix)

        # Compute std
        denoised_var = vmap(jax.scipy.linalg.cho_solve, in_axes=((0, None), 0))(
            (cholesky_tuples[0], False), k_x_X
        )
        var = vmap(self.kernel, in_axes=(None, None, 0))(
            cat_input, cat_input, dynamics_model.params
        ) - vmap(jnp.dot)(k_x_X, denoised_var)
        std = jnp.sqrt(var)

        # Compute mean
        denoised_mean = vmap(jax.scipy.linalg.cho_solve, in_axes=((0, None), 1))(
            (cholesky_tuples[0], False), dot_xs
        )
        mean = vmap(jnp.dot)(k_x_X, denoised_mean)

        # Denormalize
        mean = self.normalizer.denormalize(
            mean, dynamics_model.data_stats.xs_dot_noise_stats
        )
        std = self.normalizer.denormalize_std(
            std, dynamics_model.data_stats.xs_dot_noise_stats
        )
    ...
    def loss(
        ...
        expanded_xs = vmap(self.normalizer.normalize, in_axes=(0, None))(
            expanded_xs, data_stats.xs_after_angle_layer
        )
        us = vmap(self.normalizer.normalize, in_axes=(0, None))(us, data_stats.us_stats)

        dot_xs = vmap(self.normalizer.normalize, in_axes=(0, None))(
            dot_xs, data_stats.xs_dot_noise_stats
        )
        std_dot_xs = vmap(self.normalizer.normalize_std, in_axes=(0, None))(
            std_dot_xs, data_stats.xs_dot_noise_stats
        )
        ...
        log_pdf = vmap(multivariate_normal.logpdf, in_axes=(1, None, 0))(
            dot_xs,
            jnp.zeros(
                num_points,
            ),
    ...
    def initialize_parameters(self, key):
        ...
        parameters["lengthscale"] = random.normal(
            key=key, shape=(self.x_dim, cat_input.size)
        )
    ...
    def propose_measurement_times(
        ...
        assert (
            xs_potential.shape[1] == self.x_dim and us_potential.shape[1] == self.u_dim
        )
        ...
        covariance_matrix_history = self.m_kernel_multiple_output(
            history_inputs, history_inputs, dynamics_model.params
        )
        ...
        def posterior_kernel(input_1, input_2):
            k_X_x1 = self.v_kernel_multiple_output(
                history_inputs, input_1, dynamics_model.params
            )
            k_X_x2 = self.v_kernel_multiple_output(
                history_inputs, input_2, dynamics_model.params
            )
            k_x1_x_2 = self.kernel_multiple_output(
                input_1, input_2, dynamics_model.params
            )
            return k_x1_x_2 - vmap(jnp.dot)(
                k_X_x1,
                vmap(cho_solve, in_axes=((0, None), 0))(
                    (cho_factor_k_XX[0], False), k_X_x2
                ),
            )
        ...
        expanded_xs_potential = vmap(self.normalizer.normalize, in_axes=(0, None))(
            expanded_xs_potential, dynamics_model.data_stats.xs_after_angle_layer
        )
        us_potential = vmap(self.normalizer.normalize, in_axes=(0, None))(
            us_potential, dynamics_model.data_stats.us_stats
        )
        potential_inputs = jnp.concatenate(
            [expanded_xs_potential, us_potential], axis=1
        )
        ...
        initial_variances = vmap(posterior_kernel, in_axes=(0, 0), out_axes=0)(
...
class AbstractfSVGDDynamics(AbstractDynamics):
    ...
    def _norm_control_affine_part(self, x, u, data_stats):
        return self.normalizer.normalize(
            self._control_affine_part(x, u), data_stats.xs_dot_noise_stats
        )
    ...
    def _state_dynamics_train_one(self, params, stats, x, data_stats):
        ...
        net_out, new_state = self.model.apply(
            {"params": params, **stats}, x, mutable=list(stats.keys()), train=True
        )
    ...
    def eval_index(
        ...
        x_dot = self._dynamics_eval_one(
            params_particle, stats_particle, x, u, data_stats
        )
    ...
    def _dynamics_train_one(self, params, stats, x, u, data_stats):
        assert x.shape == (self.x_dim,) and u.shape == (self.u_dim,)
        net_out, new_state = self._state_dynamics_train_one(
            params, stats, x, data_stats
        )
    ...
    def mean_train_one(self, params, stats, x, u, data_stats):
        means, new_stats = vmap(
            self._state_dynamics_train_one, in_axes=(0, 0, None, None), out_axes=0
        )(params, stats, x, data_stats)
        return (
            jnp.mean(means, axis=0) + self._norm_control_affine_part(x, u, data_stats),
            new_stats,
        )
    ...
    def mean_and_std_train_one(self, params, stats, x, u, data_stats):
        ...
        return (
            jnp.mean(means, axis=0) + self._norm_control_affine_part(x, u, data_stats),
            jnp.std(means, axis=0),
            new_stats,
        )

    def mean_eval_one(
        self, params, stats, x, u, dynamics_data: DynamicsData, data_stats
    ):
        means = vmap(
            self._state_dynamics_eval_one, in_axes=(0, 0, None, None), out_axes=0
        )(params, stats, x, data_stats)
        denormalize = self.normalizer.denormalize
        mean = jnp.mean(means, axis=0) + self._norm_control_affine_part(
            x, u, data_stats
        )
    ...
    def mean_and_std_eval_one(
        ...
        return denormalize(mean, data_stats.xs_dot_noise_stats), denormalize_std(
            std, data_stats.xs_dot_noise_stats
        )
    ...
    def sample_dynamics_model(self, params, stats, key, data_stats):
        ...
        def _one_dynamics(x, u):
            x_dot = self._dynamics_eval_one(
                tree_map(lambda x: x[sample, ...], params),
                tree_map(lambda x: x[sample, ...], stats),
                x,
                u,
                data_stats,
            )
    ...
    def dynamics_model_idx(self, params, stats, idx, data_stats):
        def _one_dynamics(x, u):
            x_dot = self._dynamics_eval_one(
                tree_map(lambda x: x[idx, ...], params),
                tree_map(lambda x: x[idx, ...], stats),
                x,
                u,
                data_stats,
            )
    ...
    def _gp_prior_log_prob(
        self, x: jnp.array, y: jnp.array, eps: float = 1e-6
    ) -> jax.Array:
    ...
    def loss(self, params, stats, xs, us, dot_xs, std_dot_xs, data_stats: DataStats):
        assert (
            xs.shape == dot_xs.shape == std_dot_xs.shape and xs.shape[0] == us.shape[0]
        )
        assert xs.shape[1] == self.x_dim and us.shape[1] == self.u_dim

        mean_batch = vmap(
            self._dynamics_train_one, in_axes=(None, None, 0, 0, None), out_axes=(0, 0)
        )
        mean_batch_ensemble = vmap(
            mean_batch,
        ...
        norm_stds = vmap(self.normalizer.normalize_std, in_axes=(0, None))(
            std_dot_xs, data_stats.xs_dot_noise_stats
        )
        norm_stds_repeated = jnp.repeat(
            norm_stds[jnp.newaxis, ...], repeats=self.num_particles, axis=0
        )

        norm_xs_dot = vmap(self.normalizer.normalize, in_axes=(0, None))(
            dot_xs, data_stats.xs_dot_noise_stats
        )
        norm_xs_dot_repeated = jnp.repeat(
            norm_xs_dot[jnp.newaxis, ...], repeats=self.num_particles, axis=0
        )

        assert x_dot_pred.shape == (self.num_particles, xs.shape[0], self.x_dim)
        assert (
            x_dot_pred.shape == norm_xs_dot_repeated.shape == norm_stds_repeated.shape
        )
        num_train_points = xs.shape[0]
        grad_post = jax.grad(self._neg_log_posterior)(
            x_dot_pred,
            x_batch,
            norm_xs_dot_repeated,
            norm_stds_repeated,
            num_train_points,
        )
        # kernel
...
class fSVGDAffineDynamics(AbstractDynamics):
    ...
    def __init__(
        ...
        self.model = MLP(
            features=self.features, output_dim=self.x_dim + self.x_dim * self.u_dim
        )
    ...
    def _dynamics_train_one(self, params, stats, x, u, data_stats: DataStats):
        ...
        expanded_x = self.angle_layer.angle_layer(x)
        expanded_x = self.normalizer.normalize(
            expanded_x, data_stats.xs_after_angle_layer
        )
        net_out, new_state = self.model.apply(
            {"params": params, **stats},
            expanded_x,
    ...
    def mean_train_one(self, params, stats, x, u, data_stats):
        means, new_stats = vmap(
            self._dynamics_train_one, in_axes=(0, 0, None, None, None), out_axes=0
        )(params, stats, x, u, data_stats)
    ...
    def mean_eval_one(
        ...
        means = vmap(
            self._dynamics_eval_one, in_axes=(0, 0, None, None, None), out_axes=0
        )(params, stats, x, u, data_stats)
        denormalize = self.normalizer.denormalize
...
class fSVGDDynamicsQuadrotorEuler(AbstractfSVGDDynamics):
    ...
    def _control_affine_part(self, x, u):
        ...
        return self.internal_state_scaling @ jnp.array(
            [
                state_dot_0,
                state_dot_1,
                state_dot_2,
