...
class Integrator:
    def __init__(self, interactor: Interactor, simulator_config: SimulatorConfig):
        ...
        self.episode_budget = (
            simulator_config.termination_config.episode_budget_running_cost
        )
        self.simulator_type = simulator_config.simulator_type
        self.simulator_dynamics = get_simulator_dynamics(
            simulator_config.simulator_type, simulator_config.scaling
        )
        self.simulator_costs = get_simulator_costs(
            simulator_config.simulator_type, simulator_config.scaling
        )
        if simulator_config.termination_config.max_state is None:
            self.max_state: jax.Array = 1e8 * jnp.ones(
                shape=(self.simulator_dynamics.state_dim,)
            )
        else:
    ...
    def evaluate(
        self, ts: jnp.ndarray, integration_data: IntegrationData
    ) -> TrajectoryData:
    ...
    def simulate_trajectory(
        ...
        traj_idx,
        events,
    ) -> Tuple[Trajectory, Trajectory, MeasurementSelection]:
        ...
        noise = sigma * random.normal(
            key=subkey, shape=obs_data.xs_dot.shape, dtype=jnp.float64
        )
        xs_dot_noise = obs_data.xs_dot + noise

        obs_traj = Trajectory(
            ts=obs_data.ts.reshape(-1),
            us=obs_data.us,
            xs=obs_data.xs,
            xs_dot_true=obs_data.xs_dot,
            xs_dot_noise=xs_dot_noise,
        )
        vis_traj = Trajectory(
            ts=vis_data.ts.reshape(-1),
            us=vis_data.us,
            xs=vis_data.xs,
            xs_dot_true=vis_data.xs_dot,
    ...
    def simulate_trajectories(
        self, ics, time_horizon, num_vis_ts, sigmas, key, events
    ) -> Tuple[List[Trajectory], List[Trajectory], List[MeasurementSelection]]:
        ...
        for i in range(num_trajectories):
            traj_obs, traj_vis, meas_selection = self.simulate_trajectory(
                ic=ics[i],
...
class ForwardEuler(Integrator):
    def __init__(self, interactor, simulator_config: SimulatorConfig):
        ...
        total_int_steps = (
            simulator_config.num_nodes * simulator_config.num_int_step_between_nodes
        )
        self.dt = T / total_int_steps
        self.ts = jnp.linspace(
            *simulator_config.time_horizon, simulator_config.num_nodes + 1
        )
        self.between_control_ts = jnp.linspace(
            self.ts[0], self.ts[1], simulator_config.num_int_step_between_nodes + 1
        )

        total_num_steps = (
            simulator_config.num_int_step_between_nodes * simulator_config.num_nodes + 1
        )
    ...
    def between_control_points(self, x, u, t) -> Tuple[chex.Array, BetweenControlState]:
        ...
        def _next_step(
            x: chex.Array, t: chex.Array
        ) -> Tuple[chex.Array, StateDerivativePair]:
        ...
        return x_last, BetweenControlState(
            ts=cur_ts, xs=state_der_pairs.xs, xs_dot=state_der_pairs.xs_dot, u=u
        )

    def integration_step(self, carry: _IntegrationCarry, _):
        u, measurement_selection, new_events = self.interactor.interact(
            carry.x, carry.t, carry.traj_idx, carry.events
        )
        x_next, between_control_state = self.between_control_points(carry.x, u, carry.t)
        new_terminate_condition = jnp.array(False)
        new_carry = _IntegrationCarry(
            x=x_next,
            t=carry.t + 1,
            events=new_events,
            terminate_condition=new_terminate_condition,
            traj_idx=carry.traj_idx,
        )
        new_y = _IntegrationData(
            xs=between_control_state.xs,
            us=between_control_state.u,
            xs_dot=between_control_state.xs_dot,
            to_take_data=new_terminate_condition,
    ...
    def fast_simulate(self, ic, time_horizon, traj_idx, events) -> _IntegrationData:
        init = _IntegrationCarry(
            x=ic,
            t=jnp.array(0),
            terminate_condition=jnp.array(False),
            traj_idx=traj_idx,
            events=events,
        )

        new_carry, to_return = scan(
            self.integration_step, init, None, length=self.simulator_config.num_nodes
        )
        return to_return
    ...
    def simulate(self, ic, time_horizon, traj_idx, events):
        ...
        hallucination_indices = vmap(check_for_negative_one)(
            integration_data.measurement_selection.proposed_ts
        )
        measurement_selection = jtu.tree_map(
            lambda x: x[hallucination_indices], integration_data.measurement_selection
        )
        ...
        us = jnp.repeat(
            us[:, None, :],
            repeats=self.simulator_config.num_int_step_between_nodes,
            axis=1,
        )

        return IntegrationData(
            xs=jnp.concatenate(integration_data.xs[to_take]),
