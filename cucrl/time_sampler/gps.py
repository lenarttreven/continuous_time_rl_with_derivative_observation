...
def select_most_uncertain_point(xs_history: jax.Array, xs_potential, kernel, noise_std):
    ...
    k_XX = kernel_m(xs_history, xs_history) + jnp.eye(xs_history.shape[0])[None, ...] * noise_std ** 2
    cho_factor_k_XX = vmap(cho_factor, in_axes=0)(k_XX)

    def uncertainty_one_point(x):
        k_Xx = kernel_v(xs_history, x)
        k_xx = kernel(x, x)
        return k_xx - vmap(jnp.dot)(k_Xx, vmap(cho_solve, in_axes=((0, None), 0))((cho_factor_k_XX[0], False), k_Xx))
...
def select_most_uncertain_points(xs_history: jax.Array, xs_potential, kernel, noise_std, num_points):
    uncertain_points = []
    for i in range(num_points):
        uncertain_point, index, uncertainties = select_most_uncertain_point(xs_history, xs_potential, kernel, noise_std)
        ...
        xs_history = jnp.concatenate([xs_history, uncertain_point.reshape(1, -1)], axis=0)
...
if __name__ == '__main__':
    ...
    def k(x1, x2):
        ...
        var = jnp.array([jnp.exp(-0.5 * jnp.linalg.norm(x1 - x2) ** 2 / 0.5 ** 2),
    ...
    sampling_points = select_most_uncertain_points(xs_history, xs_potential, k, noise_std, 20)
