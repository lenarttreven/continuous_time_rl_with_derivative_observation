...
class TimeSampler:
    ...
    def time_horizon(self, beta: chex.Array):
        if self.time_horizon_config.type == TimeHorizonType.ADAPTIVE_TRUE:
            ...
            bnds = [(0, 1)]
            out = minimize_ipopt(
                self._horizon_condition,
                x0=jnp.array(0.6),
                jac=jit(grad(self._horizon_condition)),
                options={"max_iter": 1000, "disp": 5},
                bounds=bnds,
                args=(
                    max_beta,
                    self.system_assumptions.l_f,
                    self.system_assumptions.l_pi,
                    self.system_assumptions.l_sigma,
                    self.system_assumptions.hallucination_error,
                ),
            )
            horizon = out.x
            return horizon
