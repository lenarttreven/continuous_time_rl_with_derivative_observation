...
class FSVGD:
    def __init__(
        ...
        self.svgd_kernel, self.svgd_kernel_der = self.prepare_svgd_kernel(
            h=self.bandwidth_svgd
        )
        self.model = MLP(features=features, output_dim=self.output_dim)
        self.key = random.PRNGKey(0)
        self.domain_l, self.domain_u = domain_l, domain_u
        self.tx = optax.adam(
            learning_rate=optax.piecewise_constant_schedule(1e-2, {1000: 1e-1})
        )
    ...
    def _apply_train(self, params, stats, x, data_stats: DataStatsFSVGD):
        ...
        return self.model.apply(
            {"params": params, **stats}, x, mutable=stats.keys(), train=True
        )
    ...
    def _gp_prior_log_prob(
        ...
        x = vmap(self.normalizer.normalize, in_axes=(0, None))(
            x, data_stats.input_stats
        )
        k = self.prior_kernel(x) + eps * jnp.eye(x.shape[0])
        dist = tfd.MultivariateNormalFullCovariance(jnp.zeros(x.shape[0]), k)
        return (
            jnp.mean(jnp.sum(dist.log_prob(jnp.swapaxes(y, -1, -2)), axis=-1))
            / x.shape[0]
        )
    ...
    def _nll(
        ...
        pred_raw: jax.Array,
        y_batch: jax.Array,
        train_data_till_idx: int,
        data_std_batch: jax.Array,
    ):
        assert y_batch.shape == data_std_batch.shape
        log_prob = norm.logpdf(
            y_batch[jnp.newaxis, :],
            loc=pred_raw[:, :train_data_till_idx, :],
    ...
    def _neg_log_posterior(
        ...
        train_data_till_idx: int,
        data_std_batch: jax.Array,
        data_stats: DataStatsFSVGD,
        num_train_points: Union[float, int],
    ):
        nll = self._nll(pred_raw, y_batch, train_data_till_idx, data_std_batch)
        neg_log_prior = (
            -self._gp_prior_log_prob(x_stacked, pred_raw, data_stats) / num_train_points
        )
        neg_log_post = nll + neg_log_prior
        return neg_log_post

    def _sample_measurement_points(
        self, key: jax.random.PRNGKey, num_points: int = 10
    ) -> jax.Array:
        x_domain = jax.random.uniform(
            key,
            shape=(num_points, self.input_dim),
            minval=self.domain_l,
            maxval=self.domain_u,
        )
    ...
    def loss(
        ...
        x_domain = self._sample_measurement_points(
            key, num_points=self.num_measurement_points
        )
        x_stacked = jnp.concatenate([data.xs, x_domain], axis=0)

        # likelihood
        apply_ensemble_one = vmap(
            self._apply_train, in_axes=(0, 0, None, None), out_axes=0
        )
        apply_ensemble = vmap(
            apply_ensemble_one,
        ...
        ys_batch_norm = vmap(self.normalizer.normalize, in_axes=(0, None))(
            data.ys, data_stats.output_stats
        )
        ys_std_norm = vmap(self.normalizer.normalize_std, in_axes=(0, None))(
            ys_std, data_stats.output_stats
        )

        log_likelihood, grad_post = jax.value_and_grad(self._neg_log_posterior)(
            f_raw,
            x_stacked,
            ys_batch_norm,
            train_batch_size,
    ...
    def eval_ll(
        ...
        apply_ensemble_one = vmap(
            self.apply_eval, in_axes=(0, 0, None, None), out_axes=0
        )
        apply_ensemble = vmap(
            apply_ensemble_one,
            in_axes=(None, None, 0, None),
        ...
        neg_log_prior = (
            -self._gp_prior_log_prob(data.xs, f_raw, data_stats) / num_train_points
        )
        neg_log_post = nll + neg_log_prior
    ...
    def fit_model(
        ...
        train_loader = self._create_data_loader(
            dataset, data_std, batch_size=batch_size
        )

        for step, (data_batch, data_std_batch) in enumerate(train_loader, 1):
            key, subkey = random.split(key)
            opt_state, params, stats, loss = self._step_jit(
                opt_state,
                params,
                stats,
                data_batch,
            ...
            if step % 100 == 0 or step == 1:
                nll, nll_prior, nll_posterior = self.eval_ll(
                    params,
                    stats,
                    data_batch,
                    data_stats,
                ...
                print(
                    f"Step {step}, nll_posterior: {nll_posterior}, nll_prior: {nll_prior}, nll: {nll}"
                )
    ...
    def calculate_calibration_alpha(
        self, params, stats, xs, ys, ys_stds, ps, data_stats: DataStatsFSVGD
    ) -> jax.Array:
        # We flip so that we rather take more uncertainty model than less
        test_alpha = jnp.flip(jnp.linspace(0, 10, 100)[1:])
        test_alphas = jnp.repeat(
            test_alpha[..., jnp.newaxis], repeats=self.output_dim, axis=1
        )
        errors = vmap(
    ...
    def _calibration_errors(
        self, params, stats, xs, ys, ys_stds, ps, data_stats: DataStatsFSVGD, alpha
    ) -> jax.Array:
        ps_hat = self.calculate_calibration_score(
            params, stats, xs, ys, ys_stds, ps, data_stats, alpha
        )
    ...
    def calculate_calibration_score(
        ...
        def calculate_score(x, y, y_std):
            assert x.shape == (self.input_dim,) and y.shape == y_std.shape == (
                self.output_dim,
            )
            preds = vmap(self.apply_eval, in_axes=(0, 0, None, None), out_axes=0)(
...
if __name__ == "__main__":
    ...
    data_stats = DataStatsFSVGD(
        input_stats=Stats(mean=jnp.mean(xs, axis=0), std=jnp.std(xs, axis=0)),
    ...
    test_ys_noisy = jnp.concatenate(
        [jnp.sin(test_xs), jnp.cos(test_xs)], axis=1
    ) + noise_level * random.normal(key=random.PRNGKey(0), shape=test_ys.shape)

    test_stds = noise_level * jnp.ones(shape=test_ys.shape)
    num_ps = 10
    ps = jnp.linspace(0, 1, num_ps + 1)[1:]
    alpha_best = model.calculate_calibration_alpha(
        model_params, model_stats, test_xs, test_ys, test_ys_noisy, ps, data_stats
    )

    test_ps = model.calculate_calibration_score(
        model_params,
        model_stats,
        test_xs,
        test_ys,
    ...
    preds = vmap(apply_ens, in_axes=(0, 0, None, None))(
        model_params, model_stats, test_xs, data_stats
    )
    ...
    for j in range(output_dim):
        ...
        for i in range(num_particles):
            plt.plot(
                test_xs, preds[i, :, j], label="NN prediction", color="black", alpha=0.3
            )
        plt.plot(test_xs, jnp.mean(preds[..., j], axis=0), label="Mean", color="blue")
        plt.fill_between(
            test_xs.reshape(-1),
            (
                jnp.mean(preds[..., j], axis=0) - 2 * jnp.std(preds[..., j], axis=0)
            ).reshape(-1),
            (
                jnp.mean(preds[..., j], axis=0) + 2 * jnp.std(preds[..., j], axis=0)
            ).reshape(-1),
    ...
    for j in range(output_dim):
        ...
        plt.fill_between(
            test_xs.reshape(-1),
            (
                jnp.mean(preds[..., j], axis=0)
                - 2 * alpha_best[j] * jnp.std(preds[..., j], axis=0)
            ).reshape(-1),
            (
                jnp.mean(preds[..., j], axis=0)
                + 2 * alpha_best[j] * jnp.std(preds[..., j], axis=0)
