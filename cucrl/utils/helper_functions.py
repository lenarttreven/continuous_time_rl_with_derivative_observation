...
class BetaExploration:
    ...
    def __call__(self, num_episodes):
        if self.type == BetaType.GP:
            return self.rkhs_bound + jnp.sqrt(
                2 * (self.info_gain + jnp.log(self.state_dim) / self.delta)
            )
    ...
    def update_info_gain(self, stds: jax.Array, taus: jax.Array):
        assert (
            stds.shape == taus.shape
            and stds.ndim == 2
            and stds.shape[1] == self.state_dim
        )
...
class GPFunc:
    ...
    def _compute_f_vec(self, xs, ys):
        cov_matrix = vmap(vmap(self.kernel, in_axes=(0, None)), in_axes=(None, 0))(
            xs, xs
        ) + self.eps * jnp.eye(xs.shape[0])
        centered_data = ys - vmap(self.mean)(xs)
        cov_matrix_sqrt = cho_factor(cov_matrix)
        scaled_vector = vmap(cho_solve, in_axes=(None, 1), out_axes=1)(
            cov_matrix_sqrt, centered_data
        )
    ...
    def tree_unflatten(cls, aux_data, children):
        xs, f_vec = children
        new_class = cls(
            dim_in=aux_data["dim_in"],
            dim_out=aux_data["dim_out"],
...
class RandomFunctionQuantile:
    ...
    def evaluate_function(self, xs, ys):
        ...
        assert (
            xs.ndim == ys.ndim == 2
            and xs.shape[1] == self.dim_in
            and ys.shape[1] == self.dim_out
        )
        ...
        cov_matrix_sqrt = cho_factor(cov_matrix)
        scaled_vector = vmap(cho_solve, in_axes=(None, 1), out_axes=1)(
            cov_matrix_sqrt, centered_sample
        )
...
def sample_func(
    ...
    def sampled_fun(t_eval):
        assert t_eval.shape == ()
        return MultivariateSpline(t, sample_fun)(t_eval.reshape(1)).reshape(
            n_dim,
        ) * jnp.exp(-decay_factor * t_eval)
...
class AngleLayerDynamics:
    def __init__(
        ...
        self.angles_inv_scaling = state_scaling_inv[
            jnp.array(self.angles_dim, dtype=int)
        ]
    ...
    def angle_layer(self, x):
        ...
        transformed_angles = vmap(self._transform_angle)(
            angles, self.angles_inv_scaling
        )
...
def moving_window(a, size: int):
    ...
    return vmap(lambda start: jax.lax.dynamic_slice_in_dim(a, start, size, axis=0))(
        starts
    )
...
def angle_layer_dynamics():
    angle_layer = AngleLayerDynamics(
        state_dim=1,
        state_scaling=jnp.diag(jnp.array([2])),
        control_dim=1,
        angles_dim=[0],
    )
    test_value = 0.8
    ...
    assert jnp.allclose(
        y, jnp.array([jnp.cos(test_value / 2), jnp.sin(test_value / 2)])
    )
