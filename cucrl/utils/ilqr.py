...
class QuadraticCost(NamedTuple):
    ...
    def __call__(self, x, u, k=None):
        ...
        return (
            c + c_x @ x + c_u @ u + x @ c_xx @ x / 2 + u @ c_uu @ u / 2 + u @ c_ux @ x
            if k is None
            else self[k](x)
        )
...
def rollout_state_feedback_policy(
    dynamics, policy, x0, step_range, x_nom=None, u_nom=None
):
    def scan_fn(x, k):
...
def linear_quadratic_regulator(Qs, Rs, Hs, As, Bs):
    ...
    value_functions, optimal_policy = jax.lax.scan(
        scan_fn, final_state_value, (dynamics, cost), reverse=True
    )[1]
    return (
        jax.tree_map(
...
class TotalCost(NamedTuple):
    ...
    def __call__(self, xs, us):
        ...
        return jnp.sum(
            jax.vmap(self.running_cost)(xs[:-1], us, step_range)
        ) + self.terminal_cost(xs[-1])


@jax.jit
def iterative_linear_quadratic_regulator(
    dynamics, total_cost, x0, u_guess, maxiter=100, atol=1e-3
):
    running_cost, terminal_cost = total_cost
    n, (N, m) = x0.shape[-1], u_guess.shape
    step_range = jnp.arange(N)

    xs_iterates, us_iterates = jnp.zeros((maxiter, N + 1, n)), jnp.zeros(
        (maxiter, N, m)
    )
    ...
    value_functions_iterates = QuadraticStateCost.from_pure_quadratic(
        jnp.zeros((maxiter, N + 1, n, n))
    )
    ...
    def ilqr_iteration(loop_vars):
        ...
        (c_xx, c_xu), (c_ux, c_uu) = jax.vmap(jax.hessian(running_cost, (0, 1)))(
            xs[:-1], us, step_range
        )
        v, v_x, v_xx = (
            terminal_cost(xs[-1]),
            jax.grad(terminal_cost)(xs[-1]),
            jax.hessian(terminal_cost)(xs[-1]),
        )
        ...
        value_functions, policy = jax.lax.scan(
            scan_fn,
            quadratized_terminal_cost,
            (linearized_dynamics, quadratized_running_cost),
            reverse=True,
        )[1]
        value_functions_iterates = jax.tree_map(
            lambda x, xi, xiN: x.at[i].set(jnp.concatenate([xi, xiN[None]])),
            value_functions_iterates,
            value_functions,
            quadratized_terminal_cost,
        )
...
class NonlinearDynamics:
    ...
    def __call__(self, x, u, k):
        ...
        return x + h * self.dynamics.apply(
            self.params, self.stats, x, u, self.data_stats
        )


class ILOR:
    def __init__(
        self, dynamics: AbstractDynamics, system_cost: SimulatorCostsAndConstraints
    ):
        self.dynamics = dynamics
