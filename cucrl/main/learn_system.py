...
class LearnSystem:
    def __init__(self, config: RunConfig):
        ...
        self.angle_layer = AngleLayerDynamics(
            state_dim=self.x_dim,
            control_dim=self.u_dim,
            angles_dim=config.interaction.angles_dim,
            state_scaling=self.config.data_generator.simulator.scaling.state_scaling,
        )
        self.normalizer = Normalizer(
            state_dim=self.x_dim,
            action_dim=self.u_dim,
            tracking_c=None,
            angle_layer=self.angle_layer,
        )
        init_stats_state = Stats(
            jnp.zeros(shape=(self.x_dim,)), jnp.ones(shape=(self.x_dim,))
        )
        init_stats_control = Stats(
            jnp.zeros(shape=(self.u_dim,)), jnp.ones(shape=(self.u_dim,))
        )
        ...
        init_stats_after_angle_layer = Stats(
            jnp.zeros(shape=angles_shape), jnp.ones(shape=angles_shape)
        )

        self.data_stats = DataStats(
            ts_stats=init_stats_time,
            xs_stats=init_stats_state,
            us_stats=init_stats_control,
        ...
        self.simulator_costs = get_simulator_costs(
            config.data_generator.simulator.simulator_type,
        ...
        # Prepare data_loader
        self.data_batcher = DataLoader(
            batch_size=config.optimizers.batch_size,
        ...
        self._prepare_controller(
            self.interaction, config.data_generator.data_collection.initial_conditions
        )
        # Prepare data generator
        self.data_generator = DataGenerator(
            data_generation=config.data_generator, interactor=self.policy
        )
        # Initialize parameters
        self._initialize_parameters()
        # Prepare plotter
        self.plotter = Plotter(
            state_dim=self.data_generator.state_dim,
            action_dim=self.data_generator.control_dim,
        )
        # Prepare objectives
        ...
        self.beta_exploration = BetaExploration(
            delta=0.1,
            state_dim=self.x_dim,
            rkhs_bound=1,
        ...
        self.best_possible_discrete_cost = [
            best_possible_discrete.get_optimal_cost(ic)
            for ic in self.data_generator.initial_conditions
        ]

        self.optimal_cost = self.compute_optimal_cost(
            self.interaction, self.data_generator.initial_conditions
        )
    ...
    def compute_optimal_cost(self, control_options, initial_condition):
        ...
        offline_planner = planner_class(
            ...
            dynamics=true_dynamics_wrapper,
            simulator_costs=self.simulator_costs,
            policy_config=self.interaction.policy,
        )

        true_policy = get_interactor(
            self.x_dim,
            self.u_dim,
            true_dynamics_wrapper,
            initial_condition,
            self.normalizer,
            self.angle_layer,
            control_options,
        ...
        true_data_gen = DataGenerator(
            data_generation=self.config.data_generator, interactor=true_policy
        )

        dynamics_model = DynamicsModel(
            params=self.parameters["dynamics"],
            model_stats=self.stats["dynamics"],
        ...
        true_data_gen.simulator.interactor.update(
            dynamics_model=dynamics_model, key=key
        )
        trajs = true_data_gen.generate_trajectories(key)
        costs = vmap(self.compute_cost)(
            trajs[0].visualization_data.ts,
            trajs[0].visualization_data.xs,
    ...
    def _prepare_objectives(self):
        ...
        self.values_and_grad_dynamics_training = jit(
            value_and_grad(self.dynamics_training, 0, has_aux=True)
        )
        print("Time to prepare objective builder", time.time() - time_objective_builder)

    def _prepare_offline_planner(self):
        policy_config = self.interaction.policy
        planner_class = get_offline_planner(
            policy_config.offline_planning.exploration_strategy
        )
    ...
    def prepare_dynamics_training(self):
        if self.track_wandb:
            ...
            wandb.define_metric(
                "Dynamics training/*", step_metric="x_axis/step", summary="last"
            )
            wandb.define_metric(
                "Dynamics training/iter_time", step_metric="x_axis/step", summary="mean"
            )
        ...
        @jit
        def do_step(
            step, parameters, stats, opt_state, data, data_stats, keys, num_train_points
        ):
            (
                current_loss,
                new_stats,
            ), params_grad = self.values_and_grad_dynamics_training(
                parameters, stats, data, data_stats, keys, num_train_points
            )

            updates, opt_state = tx_dynamics_training.update(
                params_grad, opt_state, parameters
            )
        ...
        def run_optimization(
            number_of_steps, params, stats, opt_state, data, data_stats, episode, keys
        ):
            self.optimize(
                do_step=do_step,
                ...
                log_name="Dynamics training/Full objective",
                episode=episode,
                keys=keys,
            )
    ...
    def optimize(
        ...
        num_train_points = NumberTrainPoints(
            dynamics=num_train_points_dynamics, matching=0, smoother=0
        )
        data_loader = self.data_batcher.prepare_loader(
            dataset=data, key=self.current_rng
        )
        ...
        for step, data_batch in enumerate(data_loader):
            ...
            loss, opt_state, new_parameters, new_model_states = do_step(
                step,
                params,
                stats,
                opt_state,
    ...
    def cost_logging(
        self,
        episode: int,
        true_data: VisualisationData,
        predicted_data: None | OfflinePlanningData,
    ):
        true_traj_costs = vmap(self.compute_cost)(
            true_data.ts, true_data.xs, true_data.us
        )
        wandb.log(
            {"episode": episode}
            | {
                "Cost/True trajectory {}".format(traj_id): float(
                    true_traj_costs[traj_id]
                )
        ...
        wandb.log(
            {"episode": episode}
            | {
                "Cost/Optimal Trajectory {}".format(traj_id): float(
                    self.optimal_cost[traj_id]
                )
                for traj_id in range(self.num_trajectories)
            }
        )
        ...
        wandb.log(
            {"episode": episode}
            | {
                "Cost/Best possible discrete time {}".format(traj_id): float(
                    self.best_possible_discrete_cost[traj_id]
                )
                for traj_id in range(self.num_trajectories)
            }
        )
        ...
        if predicted_data is not None:
            predicted_traj_costs = vmap(self.compute_cost)(
                predicted_data.ts, predicted_data.xs, predicted_data.us
            )
            wandb.log(
                {"episode": episode}
                | {
                    "Cost/Predicted trajectory {}".format(traj_id): float(
                        predicted_traj_costs[traj_id]
                    )
                    for traj_id in range(self.num_trajectories)
                }
            )
    ...
    def log_measurement_selection(
        self, episode: int, measurement_selection: MeasurementSelection
    ):
        ...
        (
            fig_measurement_selection,
            fig_measurement_selection_space,
            fig_phase,
        ) = self.plotter.plot_measurement_selection(measurement_selection)
        ...
        episode_prefix_space = (
            "Time Selection based on Hallucination Space Difference Plot" + "/"
        )
        if fig_phase is None:
            wandb.log(
                {
                    episode_prefix
                    + episode_suffix: wandb.Image(fig_measurement_selection),
                    episode_prefix_space
                    + episode_suffix: wandb.Image(fig_measurement_selection_space),
                }
            )
        else:
    ...
    def run_episodes(self, num_episodes: int, num_iter_training: int):
        (
            prepare_params_dynamics_training,
            run_optimization_dynamics_training,
        ) = self.prepare_dynamics_training()
        ...
        for episode in range(num_episodes):
            ...
            if episode == 0:
                ...
                self.data_generator.simulator.interactor.update(
                    dynamics_model=dynamics_model, key=key
                )
            ...
            xs_dot_std = (
                self.config.data_generator.data_collection.noise
                * jnp.ones_like(xs_dot_noise)
            )

            current_dynamics_data = DynamicsData(
            ...
            self.cost_logging(
                ...
                predicted_data=self.offline_planning_data,
            )
            # Pretraining
            ...
            params_train, stats_train, opt_state = prepare_params_dynamics_training(
                train_key
            )

            run_optimization_dynamics_training(
                number_of_steps=num_iter_training,
                params=params_train,
                stats=stats_train,
                opt_state=opt_state,
                data=current_data,
            ...
            calibration_dynamics_alpha = self.dynamics.calculate_calibration_alpha(
                ...
                xs=self.vector_field_data.test_pool.xs,
                us=self.vector_field_data.test_pool.us,
                xs_dot=self.vector_field_data.test_pool.xs_dot,
                xs_dot_std=self.vector_field_data.test_pool.xs_dot_std,
            )

            # Here we need to add uncertainties for the beta exploration
            stds = vmap(self.dynamics.mean_and_std_eval_one, in_axes=(None, 0, 0))(
                self.dynamics_model, current_dynamics_data.xs, current_dynamics_data.us
            )[1]
            ...
            print(
                "Time spent for trajectory optimization: {} seconds".format(
                    time.time() - start_time_trajectory_optimization
                )
            )
            self.offline_planning_data = (
                self.data_generator.simulator.interactor.offline_planning_data
            )
            if self.visualization and self.track_wandb:
                self.visualize_controller(
                    offline_planning_data=self.offline_planning_data, episode=episode
                )
    ...
    def visualize_controller(
        self, offline_planning_data: OfflinePlanningData, episode: int
    ):
        ...
        def evaluate_prediction(offline_plan: OfflinePlanningData, ts):
            spline_states = MultivariateSpline(
                offline_plan.ts.reshape(-1), offline_plan.xs
            )
            spline_controls = MultivariateSpline(
                offline_plan.ts.reshape(-1), offline_plan.us
            )
            return spline_states(ts), spline_controls(ts)

        state_prediction, control_prediction = vmap(
            evaluate_prediction, in_axes=(0, None)
        )(offline_planning_data, vis_times)

        plot_data = PlotOpenLoop(
            times=offline_planning_data.ts,
            states=offline_planning_data.xs,
            controls=offline_planning_data.us,
            visualization_times=jnp.repeat(
                vis_times[jnp.newaxis, ...], repeats=self.num_trajectories, axis=0
            ),
        ...
        data_path = os.path.join(
            "data", "control_learning_data_episode_{}.pkl".format(episode)
        )
        with open(os.path.join(wandb.run.dir, data_path), "wb") as handle:
        ...
        wandb.log(
            {
                episode_prefix
                + "TO: Control learning": wandb.Image(fig_control_learning),
            }
        )
        plt.close("all")
    ...
    def visualize_data(self, data: DataRepr, episode: int, stage: str = ""):
        ...
        dynamics_der_means_vis, dynamics_der_stds_vis = vmap(
            self.dynamics.mean_and_std_eval_batch, in_axes=(None, 0, 0)
        )(self.dynamics_model, data.visualization_data.xs, data.visualization_data.us)
        ...
        if self.offline_planning_data is not None:
            ...
            def evaluate_prediction(
                offline_plan: OfflinePlanningData, vis_data: VisualisationData
            ):
                spline_states = MultivariateSpline(
                    offline_plan.ts.reshape(-1), offline_plan.xs
                )
            ...
            pred_states, pred_control = vmap(evaluate_prediction, in_axes=(0, 0))(
                self.offline_planning_data, data.visualization_data
            )
        ...
        data_vis_path = os.path.join(
            "data", prefix + "plot_data_episode_{}.pkl".format(episode)
        )
        with open(os.path.join(wandb.run.dir, data_path), "wb") as handle:
        ...
        wandb.log(
            {
                episode_prefix
                + "DGM: Dynamics derivatives": wandb.Image(fig_dynamics_der),
