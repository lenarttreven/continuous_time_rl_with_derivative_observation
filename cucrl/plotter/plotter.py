...
class Plotter:
    ...
    @staticmethod
    def _add_traj_opt_prediction(
        ax,
        nodes_times: jnp.array,
        nodes_values: jnp.array,
        visualization_times: jnp.ndarray,
        node_predictions: jnp.array,
    ):
        ax.scatter(
            nodes_times, nodes_values, color="red", label="Prediction from NLP solver"
        )
        ax.plot(
            visualization_times,
            node_predictions,
    ...
    def plot_open_loop_learning(self, data: PlotOpenLoop):
        ...
        ax_trajectory_optimization = ax_trajectory_optimization.reshape(
        ...
        for trajectory in range(num_traj):
            ...
            for dimension in range(self.state_dim):
                ...
                ax.grid()
                self._add_traj_opt_prediction(
                    ax=ax,
                    nodes_times=data.times[trajectory].reshape(-1),
                    nodes_values=data.states[trajectory][:, dimension],
                    visualization_times=data.visualization_times[trajectory].reshape(
                        -1
                    ),
        ...
        cols = ["State dimension {}".format(col) for col in range(self.state_dim)] + [
            "Control dimension {}".format(col) for col in range(self.state_dim)
        ]
    ...
    def plot(self, data: PlotData):
        ...
        self._create_plot(
            axs=ax_states,
            plot_times=data.visualization_times,
            plot_values=None,
            plot_values_lower=None,
            plot_values_upper=None,
            q=95,
            statistics_type=Statistics.MEAN,
            observation_times=None,
            observations=None,
            gt_times=data.visualization_times,
            gt_values=data.gt_states_vis,
            space=Space.STATE,
            prediction_states=data.prediction_states,
            actual_actions=data.actual_actions,
            predicted_actions=data.predicted_actions,
        )

        # Plot dynamics derivatives
        figure_dynamics_der, ax_dynamics = plt.subplots(
            num_traj, self.state_dim, figsize=(5 * self.state_dim + 4, 4 * num_traj)
        )
        ...
        self._create_plot(
            axs=ax_dynamics,
            plot_times=data.visualization_times,
            plot_values=data.dynamics_der_means,
    ...
    def _create_plot(
        self,
        axs,
        plot_times: jax.Array | None,
        plot_values: jax.Array | None,
        plot_values_lower: jax.Array | None,
        plot_values_upper: jax.Array | None,
        q: float,
        statistics_type: Statistics,
        observation_times: Optional[List[jnp.array]],
        observations: Optional[List[jnp.array]],
        gt_times: jax.Array,
        gt_values: jax.Array,
        space: Space = Space.STATE,
        prediction_states: jax.Array | None = None,
        actual_actions: jax.Array | None = None,
        ...
        for trajectory in range(num_trajectories):
            ...
            for dimension in range(self.state_dim):
                ...
                ax.grid()
                obs_times = (
                    None if observation_times is None else observation_times[trajectory]
                )
                obs = (
                    None
                    if observations is None
                    else observations[trajectory][:, dimension]
                )
                pred = (
                    None
                    if prediction_states is None
                    else prediction_states[trajectory][:, dimension]
                )
                cur_plot_times = None if plot_times is None else plot_times[trajectory]
                cur_plot_values = (
                    None
                    if plot_values is None
                    else plot_values[trajectory][:, dimension]
                )
                cur_plot_values_lower = (
                    None
                    if plot_values_lower is None
                    else plot_values_lower[trajectory][:, dimension]
                )
                cur_plot_values_upper = (
                    None
                    if plot_values_upper is None
                    else plot_values_upper[trajectory][:, dimension]
                )
                self._add_prediction_on_plot(
                    ax=ax,
                    plot_times=cur_plot_times,
                    plot_values=cur_plot_values,
                ...
                ax.plot(
                    gt_times[trajectory],
                    gt_values[trajectory][:, dimension],
            ...
            if actual_actions is not None:
                ...
                for dimension in range(self.action_dim):
                    ...
                    pred_actions = (
                        ...
                        else predicted_actions[trajectory][:, dimension]
                    )
                    self._add_prediction_on_plot(
                        ax=ax,
        ...
        pad = 5
        if self.state_dim >= 2 or (
            actual_actions is not None and (self.state_dim + self.action_dim >= 2)
        ):
    ...
    def _add_prediction_on_plot(
        ax,
        plot_times: jax.Array,
        plot_values: jax.Array | None,
        plot_values_lower: jax.Array | None,
        plot_values_upper: jax.Array | None,
        q: float,
        statistics_type: Statistics,
        observation_times: jax.Array | None,
        observations: jax.Array | None,
        ...
        if predicted_actions is not None:
            ax.plot(
                plot_times, predicted_actions, color="green", label="Predicted control"
            )
    ...
    def plot_measurement_selection(self, measurement_selection: MeasurementSelection):
        ...
        for trajectory in range(num_traj):
            for hallucination in range(num_hallucinations):
                ...
                cur_measurement_selection = jtu.tree_map(
                    lambda x: x[trajectory, hallucination], measurement_selection
                )
        ...
        ############################################################################
        ############################################################################
        fig_space, axs_space = plt.subplots(
            num_traj,
            num_hallucinations,
            figsize=(5 * num_hallucinations + 4, 4 * num_traj),
        )
        ...
        for trajectory in range(num_traj):
            for hallucination in range(num_hallucinations):
                ...
                self._add_measurement_selection_on_plot_space(
                    ax, cur_measurement_selection
                )
        ...
        if self.state_dim == 2:
            ...
            for trajectory in range(num_traj):
                for hallucination in range(num_hallucinations):
                    ...
                    fig_phase = self._add_measurement_selection_on_plot_phase(
                        fig_phase, ax, cur_measurement_selection
                    )
                    ax.set_xlabel(r"$x_0$")
                    ax.set_ylabel(r"$x_1$")
                    if trajectory == 0 and hallucination == 0:
    ...
    def _add_measurement_selection_on_plot_phase(
        self, fig, ax, measurement_selection: MeasurementSelection
    ):
        # measurement_selection elements have the following shape:
        ...
        repeat_num = int(
            measurement_selection.potential_xs.shape[0]
            / number_of_equidistant_measurements
        )
    ...
    def _add_measurement_selection_on_plot(
        self, ax, measurement_selection: MeasurementSelection
    ):
        # measurement_selection elements have the following shape:
        # (num_measurements, element_dim)
        assert measurement_selection.potential_xs.ndim == 2
    ...
    def _add_measurement_selection_on_plot_space(
        self, ax, measurement_selection: MeasurementSelection
    ):
        # measurement_selection elements have the following shape:
        # (num_measurements, element_dim)
        ...
        # Plot the selected times
        z = jnp.concatenate(
            (measurement_selection.potential_xs, measurement_selection.potential_us),
            axis=1,
        )
        ...
        ax.plot(
            input_differences,
            jnp.zeros_like(input_differences),
            color="blue",
            alpha=0.3,
        )
        ax.scatter(
            input_differences[measurement_selection.proposed_indices],
