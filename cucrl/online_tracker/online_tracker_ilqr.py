...
class ILQROnlineTracking(AbstractOnlineTracker):
    def __init__(
        ...
        super().__init__(
            x_dim=x_dim,
            u_dim=u_dim,
            time_horizon=interaction_config.time_horizon,
            dynamics=dynamics,
            simulator_costs=simulator_costs,
        )
        self.interaction_config = interaction_config
        # Setup time
        ts_nodes = jnp.linspace(
        ...
        self.between_control_ts = jnp.linspace(
            self.all_ts[0], self.all_ts[1], policy_config.num_int_step_between_nodes
        )
    ...
    def ode(
        self, x: chex.Array, u: chex.Array, dynamics_model: DynamicsModel
    ) -> chex.Array:
        if (
            self.interaction_config.policy.online_tracking.dynamics_tracking
            == DynamicsTracking.MEAN
        ):
            x_dot_mean = self.dynamics.mean_eval_one(dynamics_model, x, u)
            return x_dot_mean
        else:
            raise NotImplementedError(
                f"Unknown dynamics tracking: "
    ...
    def dynamics_fn(
        ...
        u_k: chex.Array,
        k: chex.Array,
        dynamics_ilqr: DynamicsILQR,
    ):
        # t will go over array [0, ..., num_nodes - 1]
        assert (
            x_k.shape == (self.x_dim,) and u_k.shape == (self.u_dim,) and k.shape == ()
        )
        ...
        def _next_step(x: chex.Array, t: chex.Array) -> Tuple[chex.Array, chex.Array]:
            x_dot = self.ode(
                x, u_k + dynamics_ilqr.us_track[k], dynamics_ilqr.dynamics_model
            )
    ...
    def cost_fn(self, x_k, u_k, k, cost_ilqr: CostILQR):
        ...
        def running_cost(x, u, t):
            ...
            assert cs.shape == (
                self.interaction_config.policy.num_int_step_between_nodes,
            )
            return jnp.sum(cs)
    ...
    def track_online(
        self,
        dynamics_model: DynamicsModel,
        initial_conditions: chex.Array,
        mpc_params: MPCParameters,
        tracking_data: TrackingData,
        t_start_idx: chex.Array,
    ) -> OCSolution:
        ...
        xs_track, us_track, ts_track = self.prepare_tracking_data(
            tracking_data, t_start_idx
        )
        ...
        cost_ilqr = CostILQR(
            xs_track=xs_track, ts_track=ts_track, dynamics_model=dynamics_model
        )

        results = self.ilqr.solve(
            cost_ilqr,
            dynamics_ilqr,
            initial_conditions,
            self.u_init,
        ...
        return OCSolution(
            ts_track, xs, us_o + delta_us, results.obj, mpc_params.dynamics_id
        )
