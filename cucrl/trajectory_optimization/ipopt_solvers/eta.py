...
class EtaSolver:
    def __init__(self, objective, gradient, eq_constrains, eq_jacobian, ineq_simulator,
                 ineq_simulator_jac, ineq_eta, ineq_eta_jac):
    ...
    def tree_flatten(self):
        children = (self.dynamics_model, self.initial_conditions, self.params_for_opt, self.jac_discovery_key,
                    self.indices)
        aux_data = {'objective': self._objective,
    ...
    def tree_unflatten(cls, aux_data, children):
        dynamics_model, initial_conditions, params_for_opt, jac_discovery_key, indices = children
        new_class = cls(objective=aux_data['objective'], gradient=aux_data['gradient'],
                        eq_constrains=aux_data['eq_constrains'], eq_jacobian=aux_data['eq_jacobian'],
                        ineq_simulator=aux_data['ineq_simulator'], ineq_simulator_jac=aux_data['ineq_simulator_jac'],
                        ineq_eta=aux_data['ineq_eta'], ineq_eta_jac=aux_data['ineq_eta_jac'])
    ...
    def _prepare_jacobian_structure(self):
        ...
        random_params_for_opt = random.normal(key=subkey, shape=self.params_for_opt.shape)
    ...
    def prepare_lower_bound(self):
        num_eq_cons = self._eq_eta(self.params_for_opt, self.initial_conditions, self.dynamics_model).size
        num_ineq_cons_sim = self._ineq_simulator(self.params_for_opt).size
        num_ineq_cons_eta = self._ineq_eta(self.params_for_opt, self.dynamics_model.beta).size
        cl = jnp.zeros(shape=(num_eq_cons + num_ineq_cons_sim + num_ineq_cons_eta,))
        cu = jnp.concatenate(
            [jnp.zeros(shape=(num_eq_cons,)), 2.0e19 * jnp.ones(shape=(num_ineq_cons_sim + num_ineq_cons_eta,))])
        return cl, cu
