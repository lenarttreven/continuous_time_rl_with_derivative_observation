    ...
    def prepare_plan(self, solve_fun: SolveFun):
        def _plan(
            ...
            def plan_one_traj(traj_key, x0, dyn_model: DynamicsModel):
                keys = random.split(
                    traj_key,
                    self.interaction_config.policy.offline_planning.num_independent_runs,
                )
                solver_results = vmap(solve_fun, in_axes=(0, None, None))(
                    keys, x0, dyn_model
                )
                best_index = jnp.argmin(solver_results.opt_value)
                solver_result = tree_map(lambda x: x[best_index], solver_results)
                return OfflinePlanningData(
                    ts=solver_result.ts,
                    xs=solver_result.xs,
                    us=solver_result.us,
            ...
            return vmap(plan_one_traj, in_axes=(0, 0, None))(
                traj_keys, initial_conditions, dynamics_model
            )
...
def get_planner(
    offline_planner: AbstractOfflinePlanner, control_config: InteractionConfig
):
