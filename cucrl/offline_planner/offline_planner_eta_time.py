...
class EtaTimeOfflinePlanner(AbstractOfflinePlanner):
    def __init__(
        ...
        self.between_control_ts = jnp.linspace(
            self.ts[0], self.ts[1], policy_config.num_int_step_between_nodes
        )
    ...
    def dynamics_fn(self, x, u, t, dynamics_model: DynamicsModel):
        assert (
            x.shape == (self.x_dim,)
            and u.shape == (self.u_dim + self.x_dim,)
            and t.shape == ()
        )
    ...
    def cost_fn(self, x, u, t, dynamics_model: DynamicsModel):
        ...
        def running_cost(_x, _u, _t):
            ...
            def _next_step(
                _x_: chex.Array, _t_: chex.Array
            ) -> Tuple[chex.Array, chex.Array]:
                x_dot = self.ode(
                    _x_, _u[: self.u_dim], _u[self.u_dim :], dynamics_model
                )
                x_next = _x_ + self.dt * x_dot
                return x_next, self.dt * self.simulator_costs.running_cost(
                    _x_, _u[: self.u_dim]
                )
    ...
    def plan_offline(
        self, dynamics_model: DynamicsModel, key: chex.PRNGKey, x0: chex.Array
    ) -> OCSolution:
        initial_actions = jnp.zeros(
            shape=(self.num_control_nodes, self.u_dim + self.x_dim)
        )
        match self.minimize_method:
            case MinimizationMethod.ILQR_WITH_CEM:
                results = self.optimizer.solve(
                    ...
                    ilqr_hyperparams=self.ilqr_params,
                    cem_hyperparams=self.cem_params,
                    random_key=key,
                )
            ...
            case _:
                raise NotImplementedError(
                    f"Minimization method {self.minimize_method} not implemented"
                )
