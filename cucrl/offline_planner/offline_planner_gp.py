...
class GPOfflinePlanner(AbstractTrajectoryOptimization):
    def __init__(
        ...
        self.example_OCSolution = OCSolution(
            times=self.time,
        ...
        self.solver = EtaSolver(
            self.objective,
            self.jac_objective,
            self.equality_dynamics,
            self.jac_equality_dynamics,
            self.inequality_simulator,
            self.jac_inequality_simulator,
            self.inequality_eta,
            self.jac_inequality_eta,
        )

        if self.exploration_strategy != ExplorationStrategy.OPTIMISTIC_ETA:
            raise TypeError(
                "This algorithm works only with ExplorationStrategy.OPTIMISTIC_ETA"
            )
    ...
    def prepare_rfq(self):
        ...
        rfq = RandomFunctionQuantile(
            dim_in=self.x_dim + self.u_dim,
            dim_out=self.x_dim,
            mu=mean,
            k=kernel,
            quantile=self.alpha,
            outer_dim_norm=Norm.L_INF,
        )
        gp_fun = GPFunc(
            dim_in=self.x_dim + self.u_dim, dim_out=self.x_dim, mean=mean, kernel=kernel
        )
        return rfq, gp_fun
    ...
    def get_states_and_controls(self, parameters):
        reshaped_parameters = parameters[self.x_dim * self.num_nodes :].reshape(
            self.num_nodes, self.x_dim + self.u_dim
        )
    ...
    def prepare_dynamics(self):
        @jit
        def system_dynamics(
            params, model_states, states, actions, data_stats, key
        ) -> Tuple[jnp.ndarray, jnp.ndarray]:
            x_dot_mean, x_dot_std = self.dynamics.mean_and_std_eval_one(
                params, model_states, states, actions, data_stats
            )
    ...
    def prepare_equality_constraints_eta(self):
        ...
        def equality(
            ...
            real_der_mean, real_der_std = vmap(
                self.system_dynamics, in_axes=(None, None, 0, 0, None, None)
            ...
            return jnp.concatenate(
                [
                    initial_condition_constraint.reshape(-1),
                    derivative_constraint.reshape(-1),
                ]
            )
    ...
    def prepare_inequality_constraints_eta(self):
        if self.exploration_norm == Norm.L_INF:
            ...
            raise NotImplementedError(
                "This exploration norm hasn't been implemented yet."
            )
    ...
    def _solve(
        ...
        # Update gp_func
        self.gp_func.prepare_function_vec(
            xs=jnp.concatenate([states, actions], axis=1), ys=eta
        )
        return OCSolution(
            self.time,
            states,
            actions,
            self.get_params_without_eta(x),
            jnp.array(info["obj_val"]),
            eta=eta,
            dynamics_key=jnp.ones(shape=(2,), dtype=jnp.uint32),
            dynamics_idx=jnp.ones(shape=(), dtype=jnp.uint32),
        )
    ...
    def solve(
        ...
        if initial_parameters.use_eta:
            ...
            eta = random.normal(
                key=initial_parameters.eta_key, shape=(self.num_nodes * self.x_dim,)
            )
