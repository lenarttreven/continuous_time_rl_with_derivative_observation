...
class MeanOfflinePlanner(AbstractOfflinePlanner):
    def __init__(
        ...
        self.ts = jnp.linspace(
            self.time_horizon[0], self.time_horizon[1], self.num_control_nodes + 1
        )
    ...
    def plan_offline(
        ...
        dynamics_id = DynamicsIdentifier(
            key=self.example_dynamics_id.key,
            idx=self.example_dynamics_id.idx,
            eta=self.example_dynamics_id.eta,
        )
        return OCSolution(
            ts=self.ts, xs=out.xs, us=us, opt_value=out.obj, dynamics_id=dynamics_id
        )
