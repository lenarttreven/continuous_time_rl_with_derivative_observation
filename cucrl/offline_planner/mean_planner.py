...
class MeanOfflinePlanner(AbstractOfflinePlanner):
    def __init__(self, state_dim: int, control_dim: int, num_nodes: int, time_horizon: Tuple[float, float],
        ...
        self.num_total_params = self.control_dim * self.num_control_nodes
        ...
        self.control_low = -10.0 * jnp.ones(shape=(self.control_dim,))
        self.control_high = 10.0 * jnp.ones(shape=(self.control_dim,))
    ...
    def plan_offline(self, dynamics_model: DynamicsModel, initial_parameters: OfflinePlanningParams,
        ...
        initial_actions = jnp.zeros(shape=(self.num_control_nodes, self.control_dim))
