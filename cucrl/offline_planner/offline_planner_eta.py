...
class EtaOfflinePlanner(AbstractOfflinePlanner):
    ...
    def get_states_and_controls(self, parameters):
        reshaped_parameters = parameters[self.x_dim :].reshape(
    ...
    def prepare_dynamics(self):
        @jit
        def system_dynamics(
            dynamics_model: DynamicsModel, states: jax.Array, actions: jax.Array
        ) -> Tuple[jax.Array, jax.Array]:
            x_dot_mean, x_dot_std = self.dynamics.mean_and_std_eval_one(
                dynamics_model, states, actions
            )
    ...
    def prepare_equality_constraints_dynamics(self):
        ...
        def equality(parameters, initial_conditions, dynamics_model: DynamicsModel):
            ...
            real_der_mean, real_der_std = vmap(
                self.system_dynamics, in_axes=(None, 0, 0)
            )(dynamics_model, states, action)
