...
for index, mss in enumerate(MSSs):
    ...
    groupped_optimistic = data_optimistic.groupby(["episode"])[
        ["Cost/True trajectory 0"]
    ]

    MEAN_costs_means = groupped_mean.median().values.reshape(-1) - SUBSRTACTION
    MEAN_costs_lower_quantile = (
        groupped_mean.quantile(q).values.reshape(
            -1,
        )
        - SUBSRTACTION
    )
    MEAN_costs_upper_quantile = (
        groupped_mean.quantile(1 - q).values.reshape(
            -1,
        )
        - SUBSRTACTION
    )
    ...
    ax.fill_between(
        np.arange(len(MEAN_costs_means)),
        MEAN_costs_lower_quantile,
        MEAN_costs_upper_quantile,
        alpha=0.3,
    )

    OPTIMISTIC_costs_means = (
        groupped_optimistic.median().values.reshape(-1) - SUBSRTACTION
    )
    OPTIMISTIC_costs_lower_quantile = (
        groupped_optimistic.quantile(q).values.reshape(
            -1,
        )
        - SUBSRTACTION
    )
    OPTIMISTIC_costs_upper_quantile = (
        groupped_optimistic.quantile(1 - q).values.reshape(
            -1,
        )
        - SUBSRTACTION
    )
    ...
    ax.plot(
        np.arange(len(OPTIMISTIC_costs_means)), OPTIMISTIC_costs_means, label="OPTIMISM"
    )
    ax.fill_between(
        np.arange(len(OPTIMISTIC_costs_means)),
        OPTIMISTIC_costs_lower_quantile,
        OPTIMISTIC_costs_upper_quantile,
        alpha=0.3,
    )
