...
for group in groups:
    ...
    plt.plot(np.arange(len(costs_means)), costs_means, label=group.split('.', 1)[1].replace('_GREEDY', ''))
    plt.fill_between(np.arange(len(costs_means)), costs_lower_quantile, costs_upper_quantile, alpha=0.3)

plt.axhline(20.16 - SUBSTRACTION, xmin=0, xmax=20, color='black', linestyle='--', label='Best continuous',
            linewidth=4)
plt.axhline(20.593 - SUBSTRACTION, xmin=0, xmax=20, color='black', linestyle='dotted', label='Best discrete',
            linewidth=4)
